...
//   SMKN 4 Kendal
    ...
    desc: "High Scholl - RPL (Rekayasa Perangkat Lunak)"
  },
];
        ...
        <div className="grid gap-10 md:grid-cols-12">
          ...
          <div className="md:col-span-9">
            <div className="space-y-12">
              {edcuation.map((edu) => (
                <div key={edu.scholl_name} className="space-y-2">
                  {/* Title + date */}
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-baseline sm:justify-between">
                    <h3 className="text-lg font-semibold text-zinc-100">
                      {edu.scholl_name}
                    </h3>
                    <p className="text-sm text-zinc-400">{edu.dateRange}</p>
                  </div>

                  {/* Location */}
                  <p className="text-sm font-medium text-sky-400">{edu.location}</p>
