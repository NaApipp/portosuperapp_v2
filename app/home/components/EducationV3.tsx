...
const experiences: ExperienceItem[] = [
  {
    id: 1,
    school_name: "Dian Nuswantoro University",
    location: "Semarang",
    date: "Aug 2026 - Present",
    description: "Currently pursuing a Bachelor of Informatics with a focus on applied software engineering, advanced programming, and system development. Actively engaged in hackathons and coding challenges to enhance problem-solving skills and explore emerging technologies.",
    skills: ["Computer Science", "Software Engineering", "Problem Solving"]
  },
  {
    id: 2,
    school_name: "SMKN 4 Kendal",
    location: "Kendal",
    date: "Jan 2024 - April 2026",
    description: "Focused on the fundamentals of software engineering, web development basics, and database management.",
    skills: ["Computer Science", "Software Engineering", "Problem Solving", "ITC Participant"]
  },
]

export default function EducationV3() {
  ...
  return (
    <section id="education" className="w-full max-w-7xl mx-auto py-24 md:py-32 cursor-default bg-background relative border-t border-text-secondary/10" ref={containerRef}>
      <FadeDown>
        <div className="max-w-7xl mx-auto px-6 md:px-12 mb-16 md:mb-24 w-full text-left">
          <h2 className="text-sm font-bold tracking-[0.2em] text-text-secondary uppercase mb-4">Education Path</h2>
          ...
          return (
            <motion.div key={exp.id} initial={{ opacity: 0, y: 40, filter: "blur(5px)" }} whileInView={{ opacity: 1, y: 0, filter: "blur(0px)" }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8, delay: index * 0.1 }} className="group/item relative grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-8 p-6 md:p-8 -mx-6 md:-mx-8 rounded-2xl transition-all duration-500 hover:!opacity-100 hover:!blur-none group-hover/list:opacity-40 group-hover/list:blur-[2px] hover:bg-text-secondary/5 hover:shadow-lg border border-transparent hover:border-text-secondary/10">
              ...
              <div className="md:col-span-3 flex flex-col">
                <h4 className="text-2xl font-bold text-text-primary tracking-tight mb-1 group-hover/item:text-text-primary transition-colors">{exp.school_name}</h4>
                <h5 className="text-sm font-bold text-text-secondary tracking-wide uppercase mb-6">{exp.location}</h5>
